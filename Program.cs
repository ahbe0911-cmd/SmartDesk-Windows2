...
<style>
*{box-sizing:border-box}body{margin:0;height:100vh;overflow:hidden;background:radial-gradient(circle at 15% 0,#315346,#07130f 45%);color:#fff;font-family:Tahoma,sans-serif;padding:18px}
...
.grid{height:calc(100% - 82px);margin-top:16px;display:grid;grid-template-columns:320px 1fr;gap:16px}.card{padding:18px}.links{display:grid;grid-template-columns:1fr 1fr;gap:10px}
...
.clock{font-size:58px;font-weight:bold;direction:ltr}.date{color:#bdd0c6;margin-top:8px}.note{width:100%;height:calc(100% - 145px);margin-top:20px;padding:15px;border:0;border-radius:15px;resize:none;font:14px Tahoma;line-height:2}
</style></head><body>
...
function tick(){let d=new Date();clock.textContent=d.toLocaleTimeString('fa-IR',{hour:'2-digit',minute:'2-digit'});date.textContent=new Intl.DateTimeFormat('fa-IR-u-ca-persian',{weekday:'long',year:'numeric',month:'long',day:'numeric'}).format(d)}setInterval(tick,1000);tick();render();
</script></body></html>
""";
}
